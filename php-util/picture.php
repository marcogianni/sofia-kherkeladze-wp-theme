...
function picture($options = [], $echo = true) {
	$defaults = [
		'src' => '',
		'mime' => 'jpeg',
		'class' => '',
	...
	if ($src_type === 'array') {
		foreach ($src as $media => $s) {
			if ($media !== 'other') {
				if ($webp) {
					$modern .= "<source {$prefix}srcset=\"$s.webp\" type=\"image/webp\" media=\"(max-width:{$media}px)\">";
				}
				$legacy .= "<source {$prefix}srcset=\"$s\" type=\"image/$mime\" media=\"(max-width:{$media}px)\">";
			} else {
				if ($webp) {
					$modern .= "<source {$prefix}srcset=\"$s.webp\" type=\"image/webp\">";
				}
		...
		if ($webp) {
			$modern .= "<source {$prefix}srcset=\"$src.webp\" type=\"image/webp\">";
		}
	...
	$html .=
		$modern .
		$legacy .
		"<img {$prefix}src=\"$original\" alt=\"$alt\" title=\"$title\"></picture>";
	if ($echo) {
		echo $html;
	} else {
		return $html;
