...
function wideo_add_complete_access_to_to_cpt(
	$singular,
	$plural,
	$role,
	$exclude = []
	...
	if (!in_array('read_' . $singular, $exclude)) {
		$users->add_cap('read_' . $singular);
	}
	if (!in_array('edit_' . $singular, $exclude)) {
		$users->add_cap('edit_' . $singular);
	}
	if (!in_array('delete_' . $singular, $exclude)) {
		$users->add_cap('delete_' . $singular);
	}
	if (!in_array('read_private_' . $plural, $exclude)) {
		$users->add_cap('read_private_' . $plural);
	}
	if (!in_array('publish_' . $plural, $exclude)) {
		$users->add_cap('publish_' . $plural);
	}
	if (!in_array('edit_' . $plural, $exclude)) {
		$users->add_cap('edit_' . $plural);
	}
	if (!in_array('edit_others_' . $plural, $exclude)) {
		$users->add_cap('edit_others_' . $plural);
	}
	if (!in_array('edit_published_' . $plural, $exclude)) {
		$users->add_cap('edit_published_' . $plural);
	}
	if (!in_array('edit_private_' . $plural, $exclude)) {
		$users->add_cap('edit_private_' . $plural);
	}
	if (!in_array('delete_' . $plural, $exclude)) {
		$users->add_cap('delete_' . $plural);
	}
	if (!in_array('delete_others_' . $plural, $exclude)) {
		$users->add_cap('delete_others_' . $plural);
	}
	if (!in_array('delete_published_' . $plural, $exclude)) {
		$users->add_cap('delete_published_' . $plural);
	}
	if (!in_array('delete_private_' . $plural, $exclude)) {
...
function wideo_edit_built_in_roles() {
	...
	$wp_roles->add_role('client', 'Gestore', [
		// general
